...
export const useStakeToken = (toToken: TokenSymbol.SOHM | TokenSymbol.GOHM) => {
  ...
  return useMutation<ContractReceipt, Error, string>(
    async amount => {
      ...
      if (parsedAmount.gt(balance)) throw new Error(t`You cannot stake more than your ${TokenSymbol.OHM} balance`);

      if (!contract) throw new Error(t`Please switch to the Ethereum network to stake your ${TokenSymbol.OHM}`);
    ...
    },
    {
      ...
      onSuccess: async () => {
        ...
        dispatch(createInfoToast(t`Successfully staked ${TokenSymbol.OHM}`));
      },
    },
  );
};
